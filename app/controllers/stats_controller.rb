class StatsController < ApplicationController
  def charts
    ...
    @durations30 = Topentry.where('duration = ? and category = ?', 30, 'duration').order('days.date').includes(:day).group_by{|e| e.user.userid}
    @distances30 = Topentry.where('duration = ? and category = ?', 30, 'distance').order('days.date').includes(:day).group_by{|e| e.user.userid}
