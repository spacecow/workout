...
describe "Day show, edit current state" do
  before(:each) do
    @user = login
    @date = '2012-07-02'
    day = FactoryGirl.create(:day, date:@date)
    create_post(date:@date)
    FactoryGirl.create(:current_state, weight:"84", day:day, user:@user)
    visit day_path(@date)
  ...
  context "create" do
    ...
    it "saves the current state to db" do
      lambda{ click_button 'Update Current State'
      }.should change(CurrentState,:count).by(0)
    end

    context "save" do
      before(:each) do
        click_button 'Update Current State'
      ...
      it "shows a flash message" do
        page.should have_notice 'Current State updated'
      end
    end
